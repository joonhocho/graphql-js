...
export function computeCost(
  ...
  // Return a Promise that will eventually resolve to the cost described by
  ...
  return new Promise(resolve => {
    ...
    return { cost, errors: context.errors };
  });
}

/**
 * Constructs a ExecutionContext object from the arguments passed to
 * computeCost, which we will pass throughout the other execution methods.
 *
  ...
  documentAST.definitions.forEach(definition => {
    switch (definition.kind) {
      ...
      default: throw new GraphQLError(
        `GraphQL cannot compute cost of a request containing a ${definition.kind}.`,
        definition
      );
    }
  });
...
function getOperationRootType(
  ...
  switch (operation.operation) {
    ...
    default:
      throw new GraphQLError(
        'Can only compute cost of queries, mutations and subscriptions',
        [ operation ]
      );
  }
}
...
function executeFields(
  ...
  const finalResults = Object.keys(fields).map(
    (responseName) => {
      const fieldASTs = fields[responseName];
      const fieldPath = path.concat([ responseName ]);
      const result = resolveField(
  ...
  if (!containsPromise) {
    return finalResults.reduce((sum, c) => sum + c, 0);
  }
  ...
  return Promise.all(finalResults)
      .then(values => values.reduce((sum, c) => sum + c, 0));
}
...
function getCostFn(fieldDef, parentType, fieldName, fieldASTs) {
  const {cost} = fieldDef;
  switch (typeof cost) {
    case 'function':
      return cost;
...
function resolveField(
  ...
  const costFn = getCostFn(fieldDef, parentType, fieldName, fieldASTs);
  ...
  // or abrupt (error).
  const result = resolveOrError(costFn, source, args, context, info);
...
// function. Returns the result of costFn or the abrupt-return Error object.
function resolveOrError(
  costFn: GraphQLFieldResolveFn,
  source: mixed,
  ...
  try {
    return costFn(source, args, context, info);
  } catch (error) {
...
function completeValue(
  ...
  if (returnType instanceof GraphQLScalarType ||
    ...
    return completeLeafValue(result);
  }
...
function completeListValue(
  ...
  */
  ...
  if (isThenable(completedItem)) {
    return completedItem.then(cost => result * Number(cost));
  }
  return result * Number(completedItem);
}
...
function completeAbstractValue(
  ...
  // TODO Iterate through all possible types.
  return 0;
  /*
